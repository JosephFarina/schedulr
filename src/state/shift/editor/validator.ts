...
function createValidator(state: RState): Validator<ShiftTemplate> {
  return {
    client: {
      invalid(val) {
    ...
    startTime: {
      invalid(val) {
        return !val || val.length <= 0 ? ['Must be a valid time range'] : null
      }
    }
  }
}
...
// function createOlffValidator(state: RState, shiftBeingValidated: Shift): Validator<Shift> {
//   return {
//     client: {
//       invalid: (val) => {
//         return !val ? ['Client must be present'] : null
